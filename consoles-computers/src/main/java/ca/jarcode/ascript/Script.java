...
public class Script {
    ...
    public static void find(Class<?> type, Object inst, FuncPool<?> pool, String prefix) {
        ...
        while (type != Object.class) {
            Arrays.stream(type.getDeclaredMethods())
                    ...
                    .map(m -> new AbstractMap.SimpleEntry<>(m.getName().substring(prefix != null ? prefix.length() : 0),
    ...
    public static ScriptValue translateToScriptValue(Object java, ScriptGlobals G) {
        ...
        // recursive
            ...
            ScriptValue[] contents = Arrays.stream(arr)
    ...
    public static PartialFunctionBind javaCallable(ScriptValue func) {
        ...
        return (args) -> {
            ScriptValue[] arr = Arrays.stream(args)
                    .map(Script::translateToScriptValue)
                    .toArray(ScriptValue[]::new);
    ...
    public static void cleanupThreadContext(ScriptGlobals G) {
        ...
        CONTEXT_FUNCS.get().get(G.getEngine())
                .values().forEach(ScriptFunction::release);
    }
