...
public final class InterpretedProgram extends SandboxProgram {
    ...
    @FunctionManual("Evaluates Lua code from a string passed as an argument. This function will " +
    ...
    public void $sound(
        ...
        delay(20);
        Sound match = Arrays.stream(Sound.values())
    ...
    public ScriptValue $require(
        ...
        if (file == null) {
            String[] arr = this.path.split("/");
            String programDir = Arrays.stream(arr)
                    .limit(arr.length - 1)
                    .collect(Collectors.joining("/"));
    ...
    public String[] $reflect() {
        return pool.functions.keySet().toArray(new String[0]);
    }
